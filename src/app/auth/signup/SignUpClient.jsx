...
export default function SignUpPage() {
  ...
  const preSelectedRoleInfo = preSelectedRole
    ? getRoleById(preSelectedRole)
    : null;
  ...
  const showRegistrationForm =
    selectedRole === ROLES.CUSTOMER || preSelectedRole;
  ...
  useEffect(() => {
    ...
    if (
      preSelectedRole &&
      [ROLES.WHOLESALER, ROLES.RESELLER].includes(preSelectedRole)
    ) {
        ...
        <div className="relative z-10 flex flex-col justify-center px-12 xl:px-20">
          <div className="mb-8">
            ...
            <p className="text-lg text-slate-400 max-w-md">
              Connect with wholesalers, resellers, and customers on a platform
              designed for growth.
            </p>
          </div>
        ...
        <div className="w-full max-w-lg py-6 sm:py-8">
          ...
          {preSelectedRoleInfo && (
            <div className="mb-6">
              ...
              <div
                className={`inline-flex items-center gap-3 px-4 py-3 rounded-xl ${preSelectedRoleInfo.bgColor} ${preSelectedRoleInfo.borderColor} border-2 w-full`}
              >
                <div
                  className={`w-10 h-10 rounded-lg bg-gradient-to-r ${preSelectedRoleInfo.color} flex items-center justify-center shadow-md`}
                >
                  <preSelectedRoleInfo.icon className="w-5 h-5 text-white" />
                </div>
                <div>
                  <div
                    className={`font-semibold ${preSelectedRoleInfo.textColor}`}
                  >
                    Registering as {preSelectedRoleInfo.label}
                  </div>
                  <div className="text-xs text-slate-500">Terms accepted ✓</div>
                </div>
              </div>
            </div>
          )}
                ...
                {roleOptions.map((role) => {
                  ...
                  return (
                    ...
                    >
                      {isSelected && (
                        <div
                          className={`absolute -top-2 -right-2 w-6 h-6 rounded-full bg-gradient-to-r ${role.color} flex items-center justify-center shadow-lg`}
                        >
                          <Check className="w-3.5 h-3.5 text-white" />
                        </div>
                      )}
                      <div
                        className={`w-10 h-10 rounded-lg bg-gradient-to-r ${role.color} flex items-center justify-center mb-3 shadow-md`}
                      >
                        <Icon className="w-5 h-5 text-white" />
                      </div>
                      <h3
                        className={`font-semibold text-sm ${
                          isSelected ? role.textColor : "text-slate-900"
                        }`}
                      >
                        {role.label}
                      </h3>
          ...
          {showRegistrationForm ? (
            ...
            <div className="text-center py-8 px-4 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <p className="text-sm text-slate-500">
                Choose how you want to use Gulu. Wholesalers and resellers will
                need to review terms first.
              </p>
