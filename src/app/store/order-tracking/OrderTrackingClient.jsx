...
import { CheckCircle2, Clock, Truck, Package, ShieldCheck, FileText } from 'lucide-react'
  ...
  return (
    <div className='container mx-auto px-4 py-8 space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        ...
        {order && (
          <Button onClick={goInvoice} variant='outline' className='border-2 hover:text-black'>
      ...
      {order && (
        <div className='space-y-6'>
          <div className='flex flex-wrap gap-6 text-sm text-muted-foreground'>
            <div>
              Order date:{' '}
              <span className='text-foreground font-medium'>{order.orderDate}</span>
            </div>
            <div>
              Estimated delivery:{' '}
                ...
                <div className='flex flex-col md:flex-row gap-6 justify-between'>
                  {timeline.map((step, idx) => (
                    <div key={idx} className='relative'>
                      <div className={`size-8 rounded-full border-2 bg-background flex items-center justify-center ${
                        step.completed ? 'border-emerald-600' : 'border-muted-foreground/30'
                      }`}>
                        {step.completed ? (
                      ...
                      <div className='text-muted-foreground text-sm'>{step.date}</div>
                    </div>
                  ))}
                </div>
              </div>
