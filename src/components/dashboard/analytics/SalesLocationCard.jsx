...
export default function SalesLocationCard({ data }) {
    ...
    datasets: [
      {
        ...
        data: countries.map((d) => ({ feature: d, value: valuesByName[d.properties.name] || 0 })),
        borderColor: '#ffffff',
        borderWidth: 0.5,
        backgroundColor: (ctx) => {
  ...
  const geoOptions = {
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (ctx) => `${ctx.raw.feature.properties.name}: ${ctx.raw.value}%`,
        },
      },
    },
    scales: {
    ...
    <Card>
      ...
      <CardContent className='space-y-3'>
        <div className='rounded-md bg-sky-50 p-4'>
          ...
          <div className='text-xs text-muted-foreground'>Top performing region this period.</div>
        </div>
          ...
          {items.map((c) => (
            <div key={c.name}>
              ...
              <div className='h-2 bg-muted rounded-md overflow-hidden'>
                <div className='h-full bg-sky-500' style={{ width: `${(c.value / max) * 100}%` }} />
              </div>
            </div>
          ))}
        </div>
