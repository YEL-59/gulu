            ...
            <div className="container mx-auto px-4 py-4">
                <nav className="flex items-center justify-between space-x-8">
                                ...
                                <MenubarContent className="w-96 [&_*]:text-gray-900 [&_*:hover]:text-gray-900 [&_*:focus]:text-gray-900">
                                    {categories.map((category) => (
                                        <MenubarSub key={category.name}>
                                            <MenubarSubTrigger className="text-gray-900 hover:text-gray-900 focus:text-gray-900">{category.name}</MenubarSubTrigger>
                                            <MenubarSubContent className="[&_*]:text-gray-900 [&_*:hover]:text-gray-900 [&_*:focus]:text-gray-900">
                                                {category.subCategories.map((subCategory) => (
                                                    <MenubarItem key={subCategory.name} asChild className="text-gray-900 hover:text-gray-900 focus:text-gray-900">
                                                        <Link href={subCategory.href} className="text-gray-900 hover:text-gray-900 focus:text-gray-900">
                                                            {subCategory.name}
                                                        </Link>
                                                    </MenubarItem>
                                                ))}
                    ...
                    <div className="flex items-center space-x-4">
                        <Link href="/wholesaler" className="text-sm text-gray-700 underline hover:text-blue-600">
                            Become a Wholesaler
                        </Link>
                        <Link href="/reseller" className="text-sm text-gray-700 underline hover:text-blue-600">
                            Become a Reseller
                        </Link>
                    </div>
                </nav>
